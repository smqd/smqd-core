   ...
   do
       ...
       case $response in
        ...
        [yY]|[yY][eE]|[yY][eE]|[sS]|[yY]|"")
            ...
            rm -rf $CLIENT_FILE_PREFIX.key.pem
            rm -rf $CLIENT_FILE_PREFIX.crt.pem
...
CERT=$(grep -m1 -n -e '-----BEGIN CERTIFICATE' $CLIENT_FILE_PREFIX.nopass.pem | cut -d: -f1) 
tail -n +$(($CERT )) $CLIENT_FILE_PREFIX.nopass.pem >> $CLIENT_FILE_PREFIX.crt.pem

head -n +$(expr $CERT - 1) \
  $CLIENT_FILE_PREFIX.nopass.pem >> $CLIENT_FILE_PREFIX.key.pem
