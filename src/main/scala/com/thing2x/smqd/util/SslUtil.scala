...
object SslUtil {
  ...
  def getSha3HashFromHexCert(hexCert: String): String = {
    ...
    val cert = Base64.getEncoder.encode(hex2bytes(hexCert)) //new BASE64Encoder().encode(hex2bytes(hexCert))
    val sha3Hash = getSha3Hash(cert.toString)
    sha3Hash
  }
  ...
  def getSha1HashFromHexCert(hexCert: String): String = {
    ...
    val cert = Base64.getEncoder.encode(hex2bytes(hexCert))
    val digest = getSha1Hash(cert.toString)
    digest
  }
  ...
  def getX509CertificateString(cert: X509Certificate): String = {
    trimNewLines(Base64.getEncoder.encode(cert.getEncoded).toString)
  }
