...
import com.thing2x.smqd.ChiefActor
...
trait ActorIdentifying extends StrictLogging {
  ...
  def identifyManagerActor(name: String)(implicit system: ActorSystem, timeout: Timeout = 2 second): ActorRef = identifyActor(manager(name))
}
